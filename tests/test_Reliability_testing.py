...
def test_chi2test():
    ...
    chisq = chi2test(distribution=dist, data=data, bins=bins, show_plot=False, print_results=False)
...
def test_KStest():
    data = mileage().failures
    dist = Normal_Distribution(mu=30011, sigma=10472)
    results = KStest(distribution=dist, data=data, print_results=False, show_plot=False)
