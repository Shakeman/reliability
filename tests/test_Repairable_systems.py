...
def test_MCF_nonparametric():
    data: list[list[int]] = [[1, 3, 5], [3, 6, 8]]
...
def test_show_time_plot() -> None:
    ...
    assert len(lines) == 2
    assert_allclose(lines[0].get_xdata(), rs._optimal_replacement_time__t)  # type: ignore
    assert_allclose(lines[0].get_ydata(), rs._optimal_replacement_time__CPUT)  # type: ignore
    assert lines[0].get_color() == "red"
    plt.close()


def test_show_ratio_plot() -> None:
